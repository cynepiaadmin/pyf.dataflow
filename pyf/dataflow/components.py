...
def all_true_longest(sources, out):
    ...
    for row in zip_longest(*sources, **dict(fillvalue=True)):
        cur_state = True
        for item in row:
...
def linear_merging(sources, out, ports=None):
    for row in zip_longest(fillvalue=Ellipsis, *sources):
        for item in row:
            yield item
