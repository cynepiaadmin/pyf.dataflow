...
def component(*port_specs, **xtra_args):
    def decor(fun):
        ...
        match = list(zip(port_specs, arg_specs))
...
def pprint_component(comp, stream=sys.stdout):
    print(comp.zf_name, file=stream)
    if comp.__doc__:
        print(comp.__doc__, file=stream)
        print('', file=stream)

    print("PORTS", file=stream)
    ...
    for port in comp.zf_port_order:
        for s, v in pmap:
            if port in v:
                print(s.ljust(10), end=' ', file=stream)
                break
        else:
            print("UNKNOWN".ljust(10), end=' ', file=stream)
        print(port, file=stream)
    if comp.zf_generator_port:
        print('', file=stream)
        print("GENERATOR PORT: %s" % comp.zf_generator_port, file=stream)
    if comp.zf_controls:
        print('\nCONTROLS', file=stream)
        print("-"*80, file=stream)
        for c in comp.zf_controls:
            print(c)

FNAME, FTYPE, FPORT, TNAME, TTYPE, TPORT = list(range(6))
EFPORT, EFPROTOCOL, EFPRIORITY, ETPORT, ETPROTOCOL, ETPRIORITY = list(range(6))
...
def control_match(map, this, values, defaults):
    ...
    """
    ctrls = {}
    for label, cdef in list(map.items()):
        for ddef, dvalue in list(defaults.items()):
            if ddef[0] == this:
                ctrls[ddef[1]] = dvalue
...
class runner(object):
        ...
        def closure():
            if name in self.in_gens:
                ...
                raise ValueError("Invalid IN port %r on component %r. "
                    "Valid Ports: %s" %
                    (name, self.comp.zf_name, list(self.in_gens.keys()) + list(self.in_gen_arrays.keys())))
    ...
    def pass_value(self, value):
        ...
        for buffer in self.out_buffer.values():
            buffer.append(value)
